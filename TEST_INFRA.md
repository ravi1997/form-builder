# E2E Test Infrastructure & Systematic Coverage Plan

This document describes the End-to-End (E2E) testing framework, directory structure, coverage matrix, and execution instructions for the Form Builder system.

---

## 1. Test Philosophy & Architecture

Our E2E testing approach is **opaque-box** and **requirement-driven**. The tests target the system's external APIs and user interface widgets to mimic real-world actor behavior without accessing or asserting private database state directly, except where verifying back-end persistence or data enforcement is strictly required.

The E2E tests are structured into **four tiers** of systematic coverage:
- **Tier 1 (Feature Coverage)**: Core happy paths (5 tests per feature).
- **Tier 2 (Boundary & Corner Cases)**: Edge cases, constraints, and validation (5 tests per feature).
- **Tier 3 (Cross-Feature Interactions)**: Pairwise interaction flows between distinct system components.
- **Tier 4 (Real-World Application Scenarios)**: Multi-stage workflows mimicking actual clinical, administrative, or compliance operations.
...
```
backend/tests/e2e/
├── conftest.py                      # Shared E2E pytest fixtures (client, db, token generation, storage mocks)
├── test_e2e_dynamic_groups.py       # Tiers 1-3 Dynamic Groups E2E tests
├── test_e2e_compliance_quota.py      # Tiers 1-3 Compliance Hold & Storage Quota E2E tests
└── test_e2e_workflows.py             # Tier 4 real-world integration workflows

frontend/test/e2e/
├── e2e_dynamic_groups_test.dart       # Tiers 1-3 Dynamic Groups UI tests
...
## 4. Tier 3: Cross-Feature Interactions

1. **F1 x F2 (Dynamic Group x Formula)**: Dynamic Group filters visibility of a Section containing calculations; verify math only runs when the qualified user fills it.
2. **F1 x F3 (Dynamic Group x Compliance)**: Restrict compliance legal hold actions to dynamic group members.
3. **F1 x F4 (Dynamic Group x Dashboard)**: Dynamic group permission restricts dashboard canvas layout editor to authorized roles.
4. **F2 x F3 (Formula x Compliance)**: Calculations compute file upload sizes, and if the total exceeds quota limits, the submission is rejected.
5. **F2 x F4 (Formula x Dashboard)**: Dashboard KPI widget binds to aggregate values computed via the AST formula engine.
6. **F3 x F4 (Compliance x Dashboard)**: Legal hold placed on a project locks dashboard canvas edits of dashboards inside that project.
...
### Python Backend E2E Tests
To run the backend E2E tests, navigate to the `backend` directory and run pytest:
```bash
cd backend
../.venv/bin/pytest tests/e2e/
```

### Flutter Frontend E2E Tests
To run the integration tests on the frontend:
```bash
cd frontend
